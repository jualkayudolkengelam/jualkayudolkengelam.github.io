...
1. **Bash-based GitHub Actions** (PRODUCTION)
   - `.github/workflows/update-product-metrics.yml` → Updates metrics with rotating schedule
   - `.github/workflows/update-post-metrics.yml` → Updates post engagement with rotating schedule
   - `.github/workflows/jekyll.yml` → Auto-deploys site
...
- ✅ Has simple Ruby script: `scripts/update-product-hybrid.rb` (manual execution, adds real reviews)
- ✅ **PRODUCTION:** Bash-based GitHub Actions workflow: `.github/workflows/update-product-metrics.yml` (automated, 2x/week)
- ✅ **PRODUCTION:** Post metrics workflow: `.github/workflows/update-post-metrics.yml` (automated, 2x/week)
- ✅ **PRODUCTION:** Deployment workflow: `.github/workflows/jekyll.yml` (auto-deploy on push)
- ✅ **Badge "Recent"** in Status column is working (generated by update-product-metrics.yml)

**Project: cmj.github**
- ❌ No automated review system
- ❌ Using fake/manual reviews in frontmatter
- ⚠️ Risk: Google penalty for self-serving fake reviews
- 📊 3 service files in `_services/`
...
- ✅ Portable (Ruby-based)
- ✅ Adds real content (testimonials)
- ✅ Natural rotation

**Limitations:**
- ⚠️ Only 6 review templates
- ⚠️ Simple rating adjustment (random -0.1 to +0.1)
- ⚠️ No smart aging algorithm
- ⚠️ Single product per run
...
### **2. Bash-based GitHub Actions** ✅ PRODUCTION

**File:** `.github/workflows/update-product-metrics.yml`
**Status:** ✅ Running in production since Nov 2025
**Schedule:** Twice per week (Tue 10:00 & Fri 12:15 WIB)

**Features:**
- ✅ Smart aging algorithm (new products get more reviews)
- ✅ Multiple products per run (probability-based: 50%)
- ✅ Multiple metrics: reviews, rating, popular flag, total_updates, last_modified_at
- ✅ Rate limiting (max +5/week)
- ✅ Auto-popular flag (80+ reviews threshold)
- ✅ Dynamic rating adjustment every 10 reviews (keep 4.5-5.0)
...
- ✅ **WORKING:** Badge "Recent" appears in Status column
- ✅ SEO freshness signals (last_modified_at updates)

**Limitations:**
- ⚠️ Bash-based (less portable than Ruby)
- ⚠️ GitHub Actions dependent (requires GitHub)
- ⚠️ No real review content added (only metrics increment)
- ⚠️ No testimonial text additions

---

## Improvement Strategy

### **Phase 1: Improve Ruby Script** ⭐ Priority
...
2. **Smart Aging Algorithm**
   ```ruby
   # New products (< 20 reviews) → Higher probability, larger increment
   # Medium products (20-50) → Medium probability
   # Mature products (> 50) → Lower probability, smaller increment
   ```
...
### **Phase 2: Setup Automation** ✅ COMPLETED

**Status:** ✅ Bash-based automation already running in production

**Existing Workflows:**
1. ✅ `.github/workflows/update-product-metrics.yml` - Product metrics (2x/week)
2. ✅ `.github/workflows/update-post-metrics.yml` - Post engagement (2x/week)
3. ✅ `.github/workflows/jekyll.yml` - Auto deployment
...
### **Phase 2.1: Rotating Schedule Implementation** ✅ COMPLETED (2025-11-24)

**Status:** ✅ Implemented for both Product and Post metrics workflows
...
### **Phase 3: Adopt to CMJ Project** ⭐ Priority
...
3. **Schema Compliance**
   - ⚠️ Avoid `AggregateRating` on Service (self-serving)
   - ✅ Keep individual `Review` items
   - ✅ Or remove schema markup entirely
   - ✅ Display reviews without structured data
...
| **Schema** | Product → OK | Service → Risky |
...
### **Week 2: Setup Automation** ✅ COMPLETED
...
- 70% → 5 stars
- 25% → 4 stars
- 5% → 4.5 stars (realistic distribution)
...
- ✅ `.github/workflows/update-product-metrics.yml` (Bash, auto-update metrics)
- ✅ `.github/workflows/update-post-metrics.yml` (Bash, auto-update post engagement)
...
1. ~~**Approach:** Ruby improvement or GitHub Actions bash?~~ → **RESOLVED:** Both! Bash for metrics (done), Ruby for real content (to improve)
2. **Review Pool:** How many reviews? What categories? → **TARGET:** 30-50 reviews across 5 categories
3. ~~**Frequency:** Weekly, bi-weekly, or monthly?~~ → **RESOLVED:** 2x/week (already running)
4. **CMJ Services:** Keep schema markup or remove? → **PENDING USER DECISION**
5. ~~**Automation:** GitHub Actions or local cron?~~ → **RESOLVED:** GitHub Actions (production)
...
**Status:** 🟢 Partially Complete (Metrics automation ✅ done, Ruby improvement 🔧 pending)
**Priority:** ⭐ Medium (automation already working, improvement is optional)
...
- Display: `/product/` page → Status column → "Recent" badge (blue)

**Two Systems Coexist:**
1. **Bash workflow** (automated) → Updates metrics only
2. **Ruby script** (manual) → Adds real review content
...
### **Rotating Schedule Implementation** ✅
